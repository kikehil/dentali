...
# Script simple para agregar las columnas faltantes directamente
# Uso: ./fix-database-simple.sh

REMOTE_USER="root"
REMOTE_HOST="147.93.118.121"
REMOTE_PATH="/var/www/html/dentali"

echo "🔧 Agregando columnas faltantes en la base de datos..."
echo ""

# Crear script SQL temporal
cat > /tmp/fix_columns.sql << 'SQL'
...
SET @preparedStatement = (SELECT IF(
  ...
  CONCAT("ALTER TABLE ", @tablename, " ADD COLUMN ", @columnname1, " DECIMAL(10, 2) NOT NULL DEFAULT 0 AFTER saldoFinalTransferencia;")
));
...
scp /tmp/fix_columns.sql $REMOTE_USER@$REMOTE_HOST:/tmp/fix_columns.sql
...
if [ -f .env ]; then
    ...
    if [ -z "$DB_NAME" ]; then
        ...
        mysql -h "$DB_HOST" -P "${DB_PORT:-3306}" -u "$DB_USER" -p"$DB_PASS" "$DB_NAME" < /tmp/fix_columns.sql
    fi
else
